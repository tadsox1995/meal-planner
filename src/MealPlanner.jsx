...
function scaleQty(qty, factor) {
  ...
  const wholeItemUnits = /^(clove|cloves|head|heads|stalk|stalks|large|medium|small|pint|pints|can|cans|bag|bags|box|boxes|slice|slices|bottle|bottles|packet|packets|jar|jars|pack|packs|sprig|sprigs|bunch|bunches|loaf|loaves|stick|sticks)/i;
...
export default function MealPlanner() {
  ...
  // Tick the cook timer every second when active
  useEffect(() => {
    if (!cookTimer) return;
  ...
  // Beep when timer reaches zero
  useEffect(() => {
    if (!cookTimer) return;
    const elapsed = Math.floor((Date.now() - cookTimer.startedAt) / 1000);
    const remaining = cookTimer.seconds - elapsed;
    if (remaining === 0) {
      try {
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (browseDetail !== null) { setBrowseDetail(null); return; }
      if (browseOpen) { setBrowseOpen(false); setBrowseSearch(""); setCameFromFinder(false); return; }
  ...
  function startCookTimer(seconds) {
    setCookTimer({ seconds, startedAt: Date.now() });
    setCookTimerTick(0);
  }
    ...
    <div style={s.app}>
      ...
      <div style={s.header}>
        ...
        <div style={{ position: "absolute", top: 14, right: 10, display: "flex", gap: 5, alignItems: "center", flexWrap: "wrap", maxWidth: "58%", justifyContent: "flex-end" }}>
          ...
          <button style={{ ...s.syncBtn, padding: "6px 9px", fontSize: 14 }} onClick={() => setBrowseOpen(true)} title="Browse all meals">{"\uD83D\uDD0D"}</button>
        ...
        {step === 2 && (
          <>
            ...
            <div
              ...
              style={{ display: "flex", alignItems: "center", gap: 12, padding: 14, background: babyMode ? "#fff4e6" : "white", border: babyMode ? "2px solid #f4c77e" : "2px solid #eee", borderRadius: 12, marginBottom: 16, cursor: "pointer" }}
            >
            ...
            {selectedMeals.map(meal => {
              ...
              return (
                <div key={meal.id} style={{ background: "white", borderRadius: 12, padding: 14, marginBottom: 10, borderLeft: "4px solid #2d6a4f", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                  ...
                  <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
                    <button onClick={() => { setBrowseDetail(meal.id); setBrowseOpen(true); }} style={{ flex: 1, padding: "8px 12px", background: "transparent", border: "1px solid #ddd", color: "#555", borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>View recipe</button>
        ...
        <div style={{ position: "fixed", inset: 0, background: "#FAFAF7", zIndex: 150, overflow: "auto", paddingBottom: 40 }}>
          <div style={{ ...s.header, position: "sticky", top: 0, zIndex: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <button onClick={() => { setBrowseOpen(false); setBrowseDetail(null); setBrowseSearch(""); setCameFromFinder(false); }} style={{ background: "rgba(255,255,255,0.2)", border: "1px solid rgba(255,255,255,0.35)", color: "white", padding: "8px 12px", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>{E.arrL} Back</button>
          ...
          {browseDetail !== null && (() => {
            ...
            return (
              <div style={{ padding: 16 }}>
                ...
                {/* Portion selector */}
                <div style={{ marginTop: 16, padding: 12, background: "#e9f5ef", borderRadius: 10, display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#2d6a4f", flex: 1 }}>Portion size</div>
                  <select
                  ...
                  >
                    {portionOpts.map(o => <option key={o} value={o}>{o} {unitLabel}</option>)}
                  </select>
                </div>
                ...
                {alreadyAdded ? (
                  ...
                  <button
                    onClick={() => { addMealToWeek(meal.id); setBrowseDetail(null); }}
                ...
                {similar.length > 0 && (
                  <>
                    ...
                    {similar.map(sm => (
                      <div key={sm.id} style={{ ...s.card(false, false), cursor: "pointer" }} onClick={() => { setBrowseDetail(sm.id); window.scrollTo({ top: 0, behavior: "smooth" }); }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ padding: 16 }}>
              ...
              {(() => {
                ...
                return filtered.map(meal => {
                  ...
                  return (
                    <div key={meal.id} style={{ ...s.card(false, false), cursor: "pointer" }} onClick={() => setBrowseDetail(meal.id)}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <span style={{ fontSize: 24 }}>
                          {meal.emoji}
                          {meal.tags.includes("quick") && <span style={{ fontSize: 14, marginLeft: 2 }}>{"\u26A1"}</span>}
          ...
          <div style={{ padding: 16 }}>
            ...
            {finderIngredients.length === 0 ? (
              ...
              return scored.map(({ meal, matchCount, matched }) => {
                const isSelected = selected.includes(meal.id);
                return (
                  <div key={meal.id} style={{ ...s.card(false, false), cursor: "pointer" }} onClick={() => { setBrowseDetail(meal.id); setBrowseOpen(true); setFinderOpen(false); setCameFromFinder(true); }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ fontSize: 24 }}>
                        {meal.emoji}
                        {meal.tags.includes("quick") && <span style={{ fontSize: 14, marginLeft: 2 }}>{"\u26A1"}</span>}
      ...
      {cookMealId !== null && (() => {
        ...
        // Compute remaining time if timer active
        let remaining = null;
        if (cookTimer) {
          const elapsed = Math.floor((Date.now() - cookTimer.startedAt) / 1000);
          remaining = Math.max(0, cookTimer.seconds - elapsed);
        }
        const fmt = (s) => {
        ...
        return (
          <div style={{ position: "fixed", inset: 0, background: "#1a1a1a", color: "white", zIndex: 200, display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ flex: 1, padding: "24px 20px", display: "flex", flexDirection: "column", justifyContent: "center", overflow: "auto" }}>
              ...
              {timerSecs && !cookTimer && (
                <button
                  onClick={() => startCookTimer(timerSecs)}
                ...
                >{"\u23F1"} Start {fmt(timerSecs)} timer</button>
              )}

              {cookTimer && (
                <div style={{ marginTop: 28, padding: 20, background: timerDone ? "#52b788" : "#2a2a2a", borderRadius: 12, textAlign: "center" }}>
                  <div style={{ fontSize: 12, color: timerDone ? "white" : "#aaa", fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 }}>
                    {timerDone ? "Time's up!" : "Timer"}
                  </div>
                  <div style={{ fontSize: 56, fontWeight: 800, fontFamily: "monospace", color: timerDone ? "white" : "#52b788", lineHeight: 1 }}>
                    {fmt(remaining)}
                  </div>
                  <button
                    onClick={stopCookTimer}
                    style={{ marginTop: 16, padding: "10px 20px", background: "transparent", border: "1px solid " + (timerDone ? "white" : "#555"), color: timerDone ? "white" : "#ccc", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: "pointer" }}
                  >Dismiss</button>
                </div>
              )}
            </div>
            ...
            <div style={{ padding: "16px 20px 28px", borderTop: "1px solid #333", display: "flex", gap: 10 }}>
              <button
                onClick={() => { if (cookStep > 0) { setCookStep(cookStep - 1); stopCookTimer(); } }}
                disabled={cookStep === 0}
              ...
              {cookStep < totalSteps - 1 ? (
                <button
                  onClick={() => { setCookStep(cookStep + 1); stopCookTimer(); }}
