...
function pickDiverseWeek(excludeIds = [], keepIds = []) {
  ...
  // Step 1: Guarantee at least ONE quick meal (skip if a keeper already covers it)
  const hasQuick = keepers.some(m => m.tags.includes("quick"));
  if (!hasQuick) {
...
export default function MealPlanner() {
  ...
  const [cookIngredientsChecked, setCookIngredientsChecked] = useState({}); // {mealId: [ingredientItem,...]}
  ...
  function shuffle() {
    ...
    // Exclude only the un-selected meals from the current week, so they don't immediately reappear
    const toExclude = R.current.week.filter(id => !keepers.includes(id));
    const fresh = pickDiverseWeek(toExclude, keepers);
    R.current.week = fresh;
    // Keep selected (which are guaranteed to be in fresh)
    setWeek(fresh);
    persist();
  }
      ...
      <div style={s.container}>
        ...
        {step === 4 && viewMode === "shop" && (
          <>
            ...
            {/* Recipe-driven aisles - REQUIRED items only - BY AISLE */}
